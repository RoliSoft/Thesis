...
\subsubsection*{ARP `WhoHas' Kérések} \label{arpping_hu}
	...
	Amikor egy kliens meg szeretné szerezni a MAC címét egy bizonyos IP címnek a hálózatban, amint \az{\ref{arpreq_hu}}. ábrán látható, készít egy ARP csomagot, amelyben kitölti a saját (ismert) adatait, illetve azokat a mezőket, amelyeket meg szeretne szerezni nullára állítja.
	...
	Az előzőleg említett ARP csomag a hálózat összes csatlakozott tagjának továbbítva van, és az a kliens, amely számára szól a kérés válaszol a küldőnek olyan módon, hogy a forrás/cél mezőket felcseréli, illetve a kinullázott mezőket behelyettesíti a saját információjával, amint ezt \az{\ref{arpresp_hu}}. ábra is mutatja. 
...
\subsubsubsection*{BSD/Darwin Implementáció}
	...
	Egy assembly-hez hasonló BPF utasításlista látható \az{\ref{bpfasm_hu}}. kódrészletben, amelyet a Wireshark nevű szoftver generál minden ARP-tól különböző csomag szűrésére.
	...
	\begin{listing}[H]
		...
		\end{minted}
		\caption{\Az{\ref{bpfasm_hu}}. Berkeley Packet Filter utasításlista lefordítása}
		\label{bpfcpp_hu}
	\end{listing}
...
\subsubsubsection*{Windows Implementáció}
	...
	Az előző fejezetekben említett Berkeley Packet Filter mechanizmus a WinPcap könyvtárcsomagban is jelen van. \Az{\ref{bpfasm_hu}}. és \ref{bpfcpp_hu}. kódrészletekben felsorolt utasításlistát csatolhatjuk a \mintinline{cpp}{pcap_setfilter(pcap_t *p, bpf_program *fp)} függvényhívással a megnyitott hálózati interfészek csatlakoztatójához.
...
\subsubsection*{Egyesítés}
	...
	A \mintinline{cpp}{PassiveScanner} komponense az alkalmazásnak kérést kezdeményez a Shodan-en[\ref{shodan_hu}] és a  Censys-en[\ref{censys_hu}] egyaránt, majd az eredményeket egybevonja a végén.
...
\subsubsection*{Identitás Kikövetkeztetése Névhírdetés Nélkül}
	...
	Míg \az{\ref{nginxregex_hu}}. kódrészletben bemutatott kifejezés elég egyszerű, a minta alapú beazonosító komponenssel olyan szervereket is be lehet azonosítani, amelyek nem hirdetik ki a nevüket a szolgáltatási sávban.
	...
	\Az{\ref{eximregex_hu}}. kódrészletben bemutatott reguláris kifejezés az ``exim'' SMTP szerver szoftvert képes beazonosítani, ugyanis az 554-es hibakód standard, az ``SMTP synchronization error'' üzenet bájtról bájtra implementáció-specifikus, és különbözik más SMTP szoftverekben.
...
\subsection*{Utolsó Frissítési Dátum Megbecslése}
	...
	\Az{\ref{opsysmatcher_hu}}. fejezetben tárgyalt komponens képes ebből beazonosítani az operációs rendszert, annak verzióját, illetve annak a verziónak a csomagkezelőjéből letölteni a PHP csomag biztonsági frissítéseinek listáját.
...
\subsection*{Adatszerző Komponensek Osztályhierarchiája}
	...
	\Az{\ref{classhierdata_hu}}. ábra bemutatja az előzőleg tárgyalt adatszerző komponensek kivitelezésének osztályhierarchiáját, illetve a közöttük levő kapcsolatokat.
	...
	\begin{figure}[!htbp]
		...
		\begin{tikzpicture}[scale=0.85,transform shape]
			...
			\node at (-6,-3) {ArpPinger$^{\ref{arpping_hu}}$};
			\node at (2,-3) {NmapScanner$^{\ref{nmapscan_hu}}$};
			...
			\node at (3.5,-6) {ShodanScanner$^{\ref{shodan_hu}}$};
			\draw  (1.5,-5.5) rectangle (5.5,-6.5);
			\node (v3_2) at (3.5,-5.5) {};
			\node at (8.5,-6) {CensysScanner$^{\ref{censys_hu}}$};
			...
			\node at (-2,-9) {TcpScanner$^{\ref{tcpscan_hu}}$};
			\node at (-6.25,-9) {IcmpScanner$^{\ref{icmpping_hu}}$};
			\node at (2,-9) {UdpScanner$^{\ref{udpscan_hu}}$};
		...
		\label{classhierdata_hu}
	\end{figure}
	\endgroup

\subsection*{Használati Példák}

	\Az{\ref{impl_hu}}. fejezetben található hivatkozás, amely a dolgozatban tárgyalt alkalmazás forráskódjára mutat, fordítás után a következő célokra használható fel. 
