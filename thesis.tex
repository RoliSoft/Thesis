...
\subsectionhu{Beállítások} \subsectionro{Setări}
	...
	A particularly useful use-case for this feature is the ability to store API keys of the supported online services (see section \ref{passive}) and lose the need to constantly resupply them as command line arguments. Listing \ref{confapi} presents the contents of a configuration file, which stores Shodan, Censys and Mr Looquer keys. All the user needs to do is to store one or more of these lines (with the proper key filled) into one of the paths listed in \ref{confpaths}.
