...
\subsection{Vulnerability Risk Analysis} \label{ssec:vulnriskanal}
	...
	The \textit{Open Web Application Security Project} has defined a risk rating methodology\cite{owasp4} specifically for vulnerabilities, wherein they define the variables as an arithmetic mean of the following factors:
	
	$ Probability = (Threat Agent + Vulnerability) / 8 $\\
	\indent\indent $ Threat Agent = Skill Level + Motive + Opportunity + Size $\\
	\indent\indent $ Vulnerability = Discovery + Exploitation + Awareness + Detection $\vspace{0.05in}\\
	\indent$ Impact = (Technical + Business) / 8 $\\
	\indent\indent $ Technical = Confidentiality + Integrity + Availability + Accountability $\\
	\indent\indent $ Business = Financial + Reputation + NonCompliance + Privacy Violation $
	
	For each factor in the above formula, a rating should be specified on the scale of $0..9$.
	
	After the ratings have been accorded and summed up, their arithmetic means need to be calculated, which then can be looked up in table \ref{scoreclass} to get the resulting classification for the \textit{probability} and \textit{impact} factors of the risk.
	
	The resulting severity classification for each factor can be looked up in table \ref{vulnrisksever} to determine the final severity of the risk.
...
\subsection{Protocol Tokenization}
	...
	\noindent The HTTP service banner shown in listing \ref{httpsvcbnr} is produced by nginx 1.9.12 with PHP 5.6.19 installed on a Ubuntu distribution. When processed by the implemented tokenizer, it will produce the elements shown in listing \ref{httpsvcbnrtokens}.
	...
	\noindent The SMTP service banner shown in listing \ref{smtpsvcbnr} is produced by Exim 4.87 listening on port 25. This banner is sent as a `greeting' as soon as the client connects to the server. This is favorable, since no further protocol probes are required to be sent in order to get a processable service banner. When this banner is processed by the implemented tokenizer, it will produce the elements shown in listing \ref{smtpsvcbnrtokens}.
