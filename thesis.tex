...
\subsectionhu{Biztonsági Tudatosság az IT Szakértőkben} \subsectionro{Conștiința de Securitate al Profesioniștilor IT}
...
\subsectionhu{Biztonsági Tudatosság az Otthoni Felhasználókban} \subsectionro{Conștiința de Securitate al Utilizatorilor Casnici}
...
\subsectionhu{Biztonsági Tudatosság a Vállalati Környezetekben} \subsectionro{Conștiința de Securitate în Mediile de Întreprindere}
...
\subsectionhu{Biztonsági Tudatosság a Pénzügyi Ágazatban} \subsectionro{Conștiința de Securitate în Sectorul Financiar}
...
\sectionhu{Szoftver Sebezhetőségek} \sectionro{Vulnerabilități de Software}
...
\subsectionhu{Sebezhetőségi Adatbázisok} \subsectionro{Bază de Date al Vulnerabilităților}
...
\subsubsectionhu{Behatolást Megelőző Rendszerek} \subsubsectionro{Sistem de Prevenire a Penetrației}
...
\subsubsection{ICMP Echo Requests}
...
\subsubsection{ARP Solicitation}
...
\subsubsubsectionhu{Platform-Független Kivitelezés Kihívásai} \subsubsubsectionro{Provocări al Implementației Platform Independente}
...
\subsubsubsectionhu{Linux Implementáció} \subsubsubsectionro{Implementația pentru Linux}
...
\subsubsubsectionhu{BSD/Darwin Implementáció} \subsubsubsectionro{Implementația pentru BSD/Darwin}

	The BSD/Darwin-specific implementation was largely based on the Linux-specific implementation, as most function calls shared the same behavior, however some subtle parametering and structure differences have crept up here and there, creating the need for a different implementation.
...
\subsubsubsectionhu{Windows Implementáció} \subsubsubsectionro{Implementația pentru Windows}
...
\subsubsubsectionhu{Protokoll Beazonosítása} \subsubsubsectionro{Identificarea Protocolului}
	...
	At this point if the server still has not sent a satisfactory reply, the server sends a TLS `ClientHello' packet in the hopes of negotiating a TLS connection. If the secure connection established, the previous plain text protocol probes are re-tried in the hopes of getting a satisfactory reply message which can by analyzed by the other components of the application.
...
\subsubsectionhu{Egyesítés} \subsubsectionro{Amalgamare}
	...
	It should be also noted that the the underlying server software is not exposed to the banned party in listing \ref{shodanban}, but the non-banned party in listing \ref{censysnoban} has their request forwarded from the load-balancer to the backend, revealing itself to be ``nginx 1.8.0,'' a version with at least one \textbf{known remotely-exploitable vulnerability}\cite{nginxcve} at this time.
...
\subsectionhu{Adatszerző Komponensek Osztályhierarhiája} \subsectionro{Ierarhia de Clasă a Componentelor Colectoare de Date}
	...
	\begin{figure}[!htbp]
		...
		\begin{tikzpicture}[scale=0.9,transform shape]
			...
			\node at (-6,-3) {ArpPinger};
			\node at (2,-3) {NmapScanner};
			\draw  (-3.5,-2.5) rectangle (-0.5,-3.5);
			\draw  (0.5,-2.5) rectangle (3.5,-3.5);
			...
			\node at (4,-6) {ShodanScanner};
			\draw  (2.5,-5.5) rectangle (5.5,-6.5);
			\node (v3_2) at (4,-5.5) {};
			\node at (8,-6) {CensysScanner};
			\draw  (6.5,-5.5) rectangle (9.5,-6.5);
			\node (v3_3) at (8,-5.5) {};
			\node (v5) at (6,-3.5) {};
			\draw [triangle 60-triangle 60] (v5) edge (v3_2);
			\draw [triangle 60-triangle 60] (v5) edge (v3_3);
			\node at (-2,-6) {ServiceScanner};
			\draw  (-3.5,-5.5) rectangle (-0.5,-6.5);
			\node at (-2,-9) {TcpScanner};
			\node at (-6,-9) {IcmpScanner};
			\node at (2,-9) {UdpScanner};
			\draw  (-3.5,-8.5) rectangle (-0.5,-9.5);
			\draw  (0.5,-8.5) rectangle (3.5,-9.5);
			\draw  (-7.5,-8.5) rectangle (-4.5,-9.5);
			\node (v3_4) at (-6,-8.5) {};
			\node (v1_1) at (-2,-6.5) {};
			\node (v2_1) at (-2,-8.5) {};
			\node (v4_1) at (2,-8.5) {};
...
\subsubsectionhu{HTTP Értelmező} \subsubsectionro{Interpretor de HTTP}
...
\subsubsectionhu{Kivitelezés Áttekintése} \subsubsectionro{Prezentarea Generală}
